...
function populateSummary(data) {
  ...
  jQuery(".pack.martopbottom span").text(
  ...
  jQuery("p.savings").text("$" + savings.toString() + "% SAVINGS APPLIED");
...
function getParamObject(urlStr) {
  ...
  let product = {
    ...
    bump: addon,
    bumpPrice: 0,
    origPrice: priceData[id].toFixed(2),
  };

  jQuery(".pull-left.elOrderProductOptinProductName input").each((i, e) => {
    if (jQuery(e).attr("value") === id) {
      product.price = parseInt(jQuery(e).attr("data-product-amount"));
